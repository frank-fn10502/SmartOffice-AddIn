...
`fetch_address_book` 必須尊重 `addressBookRequest.maxContacts`、`maxAddressEntriesPerList`、`maxGroupMembers` 與 `maxGroupDepth`。讀取 AddressLists / GAL 時不得一次無限制枚舉；展開 distribution list / group 時只回傳有限 metadata。Group 以 `isGroup=true`、`memberCount`、`memberSmtpAddresses` 與 `memberGroupSmtpAddresses` 描述，Hub 會再推導個人或子群組的 `memberOfGroupSmtpAddresses`。
