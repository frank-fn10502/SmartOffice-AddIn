...
| `fetch_address_book_roots` | 無 | 只列出 Outlook Contacts folder / AddressLists / GAL 第一層來源 |
| `fetch_address_list_entries` | `addressBookListEntriesRequest` | 依指定 AddressList 與 offset/pageSize 載入一頁通訊錄 entries |
| `fetch_address_book` | `addressBookRequest` | 相容舊式通訊錄 snapshot；不得作為預設 GAL 大量掃描流程 |
| `fetch_address_book_group_members` | `addressBookGroupMembersRequest` | 手動展開單一通訊錄 group 的 direct members |
| `address_book_relation_lookup` | `addressBookRelationLookupRequest` | 精準查詢單一 group 或個人的通訊錄關聯 |
...
| `PushAddressBookRoots` | `AddressBookRootsBatchDto` | 回推 Outlook 通訊錄第一層來源 |
| `PushAddressBookListEntriesPage` | `AddressBookListEntriesPageDto` | 回推單一 AddressList entries 分頁結果 |
| `PushAddressBookBatch` | `AddressBookBatchDto` | 分批回推 Outlook address book metadata |
| `PushAddressBookGroupMembersBatch` | `AddressBookGroupMembersBatchDto` | 分批回推單一 address book group 的 direct members |
| `PushAddressBook` | `AddressBookContactDto[]` | 回推小型 Outlook address book metadata snapshot |
...
每個 command 完成後，建議至少 invoke `ReportCommandResult`。如果 command 會改變 Outlook snapshot，請同時 invoke 對應 `Push*` method。Folder discovery 只使用 `fetch_folder_roots` 與 `fetch_folder_children`，並透過 `BeginFolderSync`、`PushFolderBatch`、`CompleteFolderSync` 回推增量結果；AddIn 不得實作一次遞迴整棵樹的 command。Folder mails 只使用 `BeginFolderMails`、`PushFolderMailsSliceResult`、`CompleteFolderMailsSlice`，且不得走 Outlook search。Mail search slice 只使用 `BeginMailSearch`、`PushMailSearchSliceResult`、`CompleteMailSearchSlice`，不要用 `PushMails` 覆蓋目前 folder list。單封屬性更新請使用 `PushMail`，不要為了更新一封 mail 重新 `PushMails`。郵件 body 請只在 `fetch_mail_body` 後以 `PushMailBody` 回推。郵件討論串請只在 `fetch_mail_conversation` 後以 `PushMailConversation` 回推。附件採 `fetch_mail_attachments` 先看 metadata、有需要才 `export_mail_attachment` 匯出到本機路徑；AddIn 不負責開檔。通訊錄 discovery 必須先用 `PushAddressBookRoots` 回推第一層來源；只有 Hub/Web UI 明確要求某個來源時，才用 `PushAddressBookListEntriesPage` 回推該頁 entries。舊式 snapshot 若仍使用 `PushAddressBookBatch`，必須固定大小分批，不得送出 50、100、150 這類累積 snapshot。
...
| `POST` | `/api/outlook/request-address-book-roots` | `fetch_address_book_roots` |
| `POST` | `/api/outlook/request-address-list-entries` | `fetch_address_list_entries` |
...
- `batchId`: string，同一次通訊錄同步的 batch id；完整 final snapshot 應使用新的 batch id。
- `sequence`: number，從 1 開始遞增。
- `reset`: boolean，第一個 full snapshot batch 應為 true，Hub 會先清空 address book snapshot 再 merge。
- `isFinal`: boolean，最後一個 full snapshot batch 應為 true。
- `totalCount`: number，這次已知的通訊錄總數；partial batch 可填目前已讀取數。
- `contacts`: `AddressBookContactDto[]`，單一 batch 的 contact 清單。AddIn 必須限制 batch 大小，避免單一 SignalR payload 過大。

通訊錄預設流程是階層式載入。`fetch_address_book_roots` 只列出第一層 Contacts folder / AddressLists / GAL 來源；不得在這一步枚舉 AddressList entries。`fetch_address_list_entries` 只處理 request 指定的單一來源與 `offset` / `pageSize`，回傳一頁 entries 與 `hasMore`。舊式 `fetch_address_book` 若為相容需求仍被呼叫，必須尊重 `addressBookRequest.maxContacts`、`maxAddressEntriesPerList`、`maxGroupMembers` 與 `maxGroupDepth`；`0` 或省略欄位代表使用安全預設，不代表無限制掃描。Group 以 `isGroup=true`、`memberCount`、`memberSmtpAddresses` 與 `memberGroupSmtpAddresses` 描述。`fetch_address_book_group_members` 只展開指定 group 的 direct members；nested group 只標示 `isGroup=true`，不得自動遞迴展開。`address_book_relation_lookup` 只解析 request 指定的單一 group 或個人：group 回傳 direct members，個人回傳所屬 Exchange distribution lists。通訊錄相關 command 必須是 read-only；不得呼叫 Outlook contact / address book 的 `Save()`、`Delete()`、`Move()`、`Items.Add()`、`Application.CreateItem()` 或任何會新增、刪除、修改通訊錄 entry 的 API。
