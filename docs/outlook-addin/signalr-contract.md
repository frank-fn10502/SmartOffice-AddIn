...
`fetch_address_book` 必須尊重 `addressBookRequest.maxContacts`、`maxAddressEntriesPerList`、`maxGroupMembers` 與 `maxGroupDepth`。讀取 AddressLists / GAL 時不得一次無限制枚舉；展開 distribution list / group 時只回傳有限 metadata。Group 以 `isGroup=true`、`memberCount`、`memberSmtpAddresses` 與 `memberGroupSmtpAddresses` 描述，Hub 會再推導個人或子群組的 `memberOfGroupSmtpAddresses`。`fetch_address_book` 必須是 read-only；不得呼叫 Outlook contact / address book 的 `Save()`、`Delete()`、`Move()`、`Items.Add()`、`Application.CreateItem()` 或任何會新增、刪除、修改通訊錄 entry 的 API。
